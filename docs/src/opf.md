...
```julia
classic = ClassicPerUnitScaling(1e6)
si = SIUnitsScaling()
...
@assert opf_scaling_policy(ctx) === custom
```

For an experimental power base that changes across an isolated transformer,
use `ZonePerUnitScaling(voltage_bases=..., power_bases=...)`. Every bus must be
...
`ConsistentPerUnitScaling` requires every AC bus to be declared and validates
...
Under `ZonePerUnitScaling`, the ampere-turn equation in model coordinates is
...
With `ZonePerUnitScaling`, this displayed physical/global-base equation becomes
...
Use `opf_initialization_data(ctx)` to inspect the equation counts and maximum
...
`opf_transformer_scaling_contract_data(ctx; voltage_bases, power_bases)` to
...
the model. When the context was built with a qualified `ZonePerUnitScaling`,
...
`opf_acdc_scaling_contract_data(ctx)`. If converter `c` is attached to AC bus
...
| `opf_semantic_blocks(ctx)` | authoritative paired coordinate/residual blocks and local reference-scale provenance |
| `register_opf_semantic_block!(ctx, block)` | extension seam for a non-overlapping semantic block; metadata only |
...
result = solve_opf(net; model_hook! = ctx -> begin
    ...
    scale = opf_ac_coordinate_bases(ctx, "bus1").power
