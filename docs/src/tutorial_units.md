...
The public policy boundary makes a base experiment explicit and reproducible.
The historical keywords remain supported and resolve to
`ClassicPerUnitScaling(s_base)`, while the equivalent staged call is:

```julia
ctx = build_opf_model(net; scaling_policy = ClassicPerUnitScaling(1e6))
```
...
policy = ConsistentPerUnitScaling(
...
@assert opf_scaling_policy(ctx) === policy
@assert opf_bases(ctx).i_base[src["bus"]] ==
        policy.s_base / custom_bases[src["bus"]]
```
...
`opf_transformer_scaling_contract_data` audits such a proposal without changing
the model:

```julia
proposal = opf_transformer_scaling_contract_data(
    ctx;
...
The first executable local-base slice is available as `ZonePerUnitScaling`:
...
evidence = opf_transformer_scaling_contract_data(ctx)
...
stable `opf_ac_coordinate_bases` and `opf_dc_coordinate_bases` helpers expose
the voltage/current/power coordinates to engine hooks without relying on
internal base-table fields. `opf_acdc_scaling_contract_data` records every
...
After `enforce_kcl!(ctx)`, `opf_semantic_blocks(ctx)` exposes the engine's
authoritative paired coordinates and residuals. Native declarations currently
...
```julia
enforce_kcl!(ctx)
blocks = opf_semantic_blocks(ctx)
...
```julia
initialization = opf_initialization_data(ctx)
