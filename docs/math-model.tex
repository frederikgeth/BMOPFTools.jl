...
$S^{\max}_{g,k}$ & VA & Generator apparent-power rating, phase $k$ (optional) & \texttt{generator.s\_max} \\
$I^{\max}_{g,k}$ & A & Generator current-magnitude limit, phase $k$ (optional) & \texttt{generator.i\_max} \\
...
$I^{\max}_{n,k}$ & A & IBR current-magnitude limit, phase $k$ (optional) & \texttt{ibr.i\_max} \\
...
Two optional per-phase ratings may additionally be supplied (omitted otherwise).
An apparent-power rating $S^{\max}_{g,k}$ stamps the power circle, and a
current-magnitude limit $I^{\max}_{g,k}$ stamps the current circle directly on
the terminal current variables:
%
\begin{align}
  P_{g,k}^2 + Q_{g,k}^2 &\;\leq\; \bigl(S^{\max}_{g,k}\bigr)^2,
  \label{eq:gen-s}\\
  \bigl(c^{r,g}_{g,k}\bigr)^2 + \bigl(c^{i,g}_{g,k}\bigr)^2
  &\;\leq\; \bigl(I^{\max}_{g,k}\bigr)^2.
  \label{eq:gen-i}
\end{align}
...
\end{align}
%
When a per-phase current-magnitude limit is supplied it is stamped directly on
the converter current variables (optional; omitted otherwise):
%
\begin{equation}
...
\end{equation}
%
Since $|S_{n,k}| = |\Delta v_k|\,|I_{n,k}|$, this caps reactive capability
$\approx$ linearly in voltage ($Q^{\max} \approx |\Delta v_k|\,I^{\max}_{n,k}$),
the faithful voltage-source-converter behaviour, rather than holding it flat at
$S^{\max}$ as the apparent-power circle alone would.
