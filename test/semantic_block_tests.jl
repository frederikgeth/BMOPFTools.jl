@testset "OPF semantic coordinate and residual blocks" begin
    ...
    enforce_kcl!(context)
    variable_blocks = opf_semantic_blocks(context; kind=:variable)
    constraint_blocks = opf_semantic_blocks(context; kind=:constraint)
    ...
    copy_blocks = opf_semantic_blocks(context)
    copy_blocks[1].metadata["mutated"] = true
    @test !haskey(opf_semantic_blocks(context)[1].metadata, "mutated")
    @test_throws ArgumentError opf_semantic_blocks(context; kind=:objective)

    overlap = OpfSemanticBlock(
        "overlap",
        :variable,
    ...
    @test_throws ArgumentError register_opf_semantic_block!(context, overlap)
    @test_throws ArgumentError OpfSemanticBlock(
        "singular",
        :variable,
        copy(voltage_block.members),
        [:real, :imag],
