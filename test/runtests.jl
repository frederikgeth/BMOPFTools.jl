...
@testset "BMOPFTools" begin
    ...
    @testset "External CC BY-NC-SA dataset integration" begin
        if isempty(get(ENV, "BMOPF_RESTRICTED_DATA", ""))
            @test_skip "Set BMOPF_RESTRICTED_DATA to BMOPFDraftData/test/data to run dataset integration tests"
        else
            include("restricted_data_tests.jl")
        end
    end
...
include("mcp_execution_tests.jl")
    ...
    @testset "OPF extension" begin
        if !_HAS_JUMP_IPOPT
            ...
            include("opf_engine_review_tests.jl")
            include("opf_device_substitution_tests.jl")
