@testset "Typed OPF scaling policies" begin
    ...
    @test SIUnitsScaling() isa AbstractOpfScalingPolicy
    @test ClassicPerUnitScaling().s_base == 1e6
    @test_throws ArgumentError ClassicPerUnitScaling(0.0)
    @test_throws ArgumentError ConsistentPerUnitScaling(
        s_base=1e6, voltage_bases=Dict("source" => -1.0))
    ...
    classic_bases = ext._compute_bases(net, ClassicPerUnitScaling(1e6))
    @test legacy_bases.s_base == classic_bases.s_base
    @test legacy_bases.v_base == classic_bases.v_base
    @test classic_bases.scaling_policy isa ClassicPerUnitScaling

    custom = ConsistentPerUnitScaling(
    ...
    missing = ConsistentPerUnitScaling(
        s_base=1e6, voltage_bases=Dict("source" => 1000.0))
    @test_throws ArgumentError ext._compute_bases(net, missing)
    mismatched = ConsistentPerUnitScaling(
        s_base=1e6,
    ...
    @test opf_scaling_policy(ctx) === custom
    ...
    current_contract = opf_transformer_scaling_contract_data(ctx)
    ...
    inconsistent_zone = opf_transformer_scaling_contract_data(
        ctx;
    ...
    si_ctx = initialize_opf_model(net; scaling_policy=SIUnitsScaling())
    @test opf_bases(si_ctx) === nothing
    @test opf_scaling_policy(si_ctx) isa SIUnitsScaling

    classic_result = solve_pf(net; scaling_policy=ClassicPerUnitScaling(1e6))
...
@testset "Zone-local per-unit scaling across an isolated transformer" begin
    ...
    @test opf_scaling_policy_data(zone)["kind"] == "zone_per_unit"
    ext = Base.get_extension(BMOPFTools, :BMOPFOpfExt)
    ...
    contract = opf_transformer_scaling_contract_data(context)
    ...
    si = solve_pf(net; scaling_policy=SIUnitsScaling())
    classic = solve_pf(net; scaling_policy=ClassicPerUnitScaling(1.0e6))
    ...
    si_opf = solve_opf(net; scaling_policy=SIUnitsScaling())
...
@testset "Zone-local Yd/Dy connection-matrix covariance" begin
    ...
    for subtype in ("wye_delta", "delta_wye")
        @testset "$subtype" begin
            ...
            classic_context = build_opf_model(
                net; scaling_policy=ClassicPerUnitScaling(1.0e6),
                add_objective=false,
            )
            voltage_bases = Dict(opf_bases(classic_context).v_base)
            zone_policy = ZonePerUnitScaling(
    ...
    guarded_policy = ZonePerUnitScaling(
...
@testset "Zone-local galvanic regulator covariance" begin
    ...
    for (label, fixture, subtype, transformer_id) in (
        ...
        @testset "$label fixed and free taps" begin
            ...
            contract = opf_transformer_scaling_contract_data(local_context)
            @test contract["proposal_admissible"]
            ...
            si = solve_pf(network; scaling_policy=SIUnitsScaling())
            ...
            si_free = build_opf_model(
                free_network; scaling_policy=SIUnitsScaling(), add_objective=false,
            )
            local_free = build_opf_model(
                free_network; scaling_policy=zone_policy, add_objective=false,
            )
            si_init = opf_initialization_data(si_free)
            local_init = opf_initialization_data(local_free)
    ...
    si_context = build_opf_model(
        ...
        scaling_policy=SIUnitsScaling(), add_objective=false,
    )
    bad_voltage = opf_transformer_scaling_contract_data(
        si_context;
...
@testset "Zone-local AC/DC converter covariance" begin
    ...
    ac1 = opf_ac_coordinate_bases(context, "f1")
    ac2 = opf_ac_coordinate_bases(context, "f2")
    dc = opf_dc_coordinate_bases(context, "dcA")
    ...
    @test_throws ArgumentError opf_ac_coordinate_bases(context, "missing")
    @test_throws ArgumentError opf_dc_coordinate_bases(context, "missing")
    contract = opf_acdc_scaling_contract_data(context)
    ...
    si = solve_opf(network; scaling_policy=SIUnitsScaling())
    ...
    si_context = build_opf_model(
        network; scaling_policy=SIUnitsScaling(), add_objective=false,
    )
    @test opf_ac_coordinate_bases(si_context, "f1").power == 1.0
    @test opf_dc_coordinate_bases(si_context, "dcA").power == 1.0
    @test opf_acdc_scaling_contract_data(si_context)[
...
@testset "Voltage warm-start scaling invariance across voltage levels" begin
    ...
    classic = ClassicPerUnitScaling(1.0e6)
    ...
    contexts = [
        build_opf_model(
            net; scaling_policy=SIUnitsScaling(), add_objective=false,
        ),
        classic_probe,
...
@testset "Compositional transformer phasor warm starts" begin
    ...
    @testset "single-phase lateral inherits its actual parent phase" begin
        ...
        evidence = opf_initialization_data(context)
    ...
    @testset "chained Yd/Dy vector-group equations" begin
        ...
        pu_evidence = opf_initialization_data(pu_context)
        ...
        local_power_contract = opf_transformer_scaling_contract_data(
            context;
            voltage_bases=Dict(
                "hv" => 6350.0,
    ...
    @testset "n-winding delta_roll coil equations" begin
        ...
        local_context = build_opf_model(
            net;
            scaling_policy=ZonePerUnitScaling(
        ...
        evidence = opf_initialization_data(local_context)
