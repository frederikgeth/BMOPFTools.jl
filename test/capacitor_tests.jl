...
@testset "capacitor — nameplate → susceptance & B-matrix" begin
    vpn = 14400.0
    # WYE: B per phase = q_rated / v_rated².
    wye = Dict{String,Any}("bus"=>"b", "terminal_map"=>["a","b","c","n"],
        "configuration"=>"WYE", "q_rated"=>[6.0e5, 6.0e5, 6.0e5], "v_rated"=>vpn)
    b = 6.0e5 / vpn^2
    ...
    sp = Dict{String,Any}("bus"=>"b", "terminal_map"=>["a","n"],
        "configuration"=>"SINGLE_PHASE", "q_rated"=>[6.0e5], "v_rated"=>vpn)
    _, Bsp = BC._cap_bmatrix(sp)
    @test Bsp ≈ [b -b; -b b]

    # DELTA: across each phase pair, line-to-line v_rated.
    vll = 24942.0
    bd  = 3.0e5 / vll^2
    dl = Dict{String,Any}("bus"=>"b", "terminal_map"=>["a","b","c"],
        "configuration"=>"DELTA", "q_rated"=>[3.0e5, 3.0e5, 3.0e5], "v_rated"=>vll)
...
@testset "capacitor — validation (completeness / spec)" begin
    good = Dict{String,Any}("bus"=>"b", "terminal_map"=>["a","b","c","n"],
        "configuration"=>"WYE", "q_rated"=>[6.0e5,6.0e5,6.0e5], "v_rated"=>14400.0)
    ...
    # missing v_rated → completeness error
    b1 = deepcopy(good); delete!(b1, "v_rated")
    ...
    # non-positive v_rated → spec error
    b4 = deepcopy(good); b4["v_rated"] = 0.0
