...
function _add_center_tap_transformer!(model, tid, xfmr, vr, vi, cr_xf, ci_xf, kcl_r, kcl_i;
    ...
    # Legacy no-load admittance across winding 2 (the first LV half-winding).
    ...
    #
    # This analytical Yprim folds N into y1 = N²/Z1(tap) and C = ±1/N, so it is only
    # used when the ratio is a FIXED number. A free tap (variable N) takes the
    # degree-2 T-model branch below, which is algebraically identical at each N
    ...
    if arm1 && arm2 && tap === nothing
        Z1 = (N / N0)^2 * (R1 + im * X1)  # nominal HV ohms, turns-scaled
    ...
    # where v1 = V_t1 − V_tn, v2 = V_tn − V_t2, Z1 = (N/N0)²(R1+jX1) and
    ...
    # collapses to the ideal V_hv = N·v1 = N·v2. Eliminating N·Is by current
    # coupling below keeps the constraint set quadratic for a free tap.
    ...
    # Primary leakage uses nominal ohms, so the physical drop is
    # (N/N0)² Z1 Is. Substitute N*f*Is = Il2-Il1 to retain degree two even
    # for a free ratio; this is also exact when either star arm is zero.
    z1r_Is = @expression(model, N / (N0^2 * current_from_factor) *
        (R1 * (Il2r - Il1r) - X1 * (Il2i - Il1i)))
    z1i_Is = @expression(model, N / (N0^2 * current_from_factor) *
        (R1 * (Il2i - Il1i) + X1 * (Il2r - Il1r)))
