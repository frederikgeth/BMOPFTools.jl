...
# in which case we bound −Δ. When the sign is unknown (no pole role) we fall back
# to the (nonconvex) squared form |Δ| ≤ hi and skip the lower bound.
function _bound_oriented_diff!(model, Δ, lo, hi, orient::Int)
    if orient == 0
        hi !== nothing && @constraint(model, Δ^2 <= Float64(hi)^2)
        return
    end
...
# above the return, NEGATIVE pole below), so the bounds are linear. Terminals with
# no declared role fall back to the squared (sign-agnostic) form.
    ...
    for (b, dcbus) in get(net, "dc_bus", Dict())
        ...
        if lllo !== nothing || llhi !== nothing
            ...
            if !isempty(pos) && !isempty(neg) &&
               haskey(v_dc, (b, pos[1])) && haskey(v_dc, (b, neg[1]))
...
"""
    _couple_converter_to_dc!(model, vars, inv_id, inv, p_ac_expr)

Lossless converter DC-port coupling: the power drawn from the DC bus equals the
converter's AC active power `p_ac_expr` (= Σ phase P). Stamps the bilinear power
balance and injects the DC-port current into the shared DC KCL, so converters on
the same dc_bus balance active power through the DC node.
"""
function _couple_converter_to_dc!(model, vars, inv_id, inv, p_ac_expr)
    haskey(vars, :kcl_dc) || return
    v_dc = vars[:v_dc]; kcl = vars[:kcl_dc]
    b  = String(get(inv, "dc_bus", "")); tm = string.(get(inv, "dc_terminal_map", String[]))
    I  = vars[:idc_conv][inv_id]
    if length(tm) >= 2
        ka = (b, tm[1]); kb = (b, tm[2])
        (haskey(v_dc, ka) && haskey(v_dc, kb)) || return
        @constraint(model, (v_dc[ka] - v_dc[kb]) * I == p_ac_expr)
        _dc_kcl_add!(kcl, b, tm[1], -I)   # current drawn from pole into converter
        _dc_kcl_add!(kcl, b, tm[2],  I)
    elseif length(tm) == 1
        ka = (b, tm[1]); haskey(v_dc, ka) || return
        @constraint(model, v_dc[ka] * I == p_ac_expr)
        _dc_kcl_add!(kcl, b, tm[1], -I)   # earth return
    end
end
